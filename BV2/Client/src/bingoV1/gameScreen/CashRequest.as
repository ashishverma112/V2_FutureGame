	...
	{ 
		...
		public function CashRequest(bgs:*,fromLooby:Boolean=false) 
		{
			...
			if (fromLooby)
			{
				...
				//screenUI.totalbalance.text = "0.0";
				screenUI.gamebalance.text = getApproxValueToPlaces(Number(_bgs._gameAmount),2).toFixed(2);
				screenUI.totalbalance.text = getApproxValueToPlaces(Number(_bgs._gameAmount) + Number(screenUI.totalbalance.text), 2).toFixed(2);
		...
		private function gotoPayment(evt:MouseEvent):void {
			...
			var str:String = GetDisplayObject.getPaymentArray();
		//trace(str);
		...
		public function addCash(str:String):void
		{
			...
			screenUI.balance.text = (getApproxValueToPlaces( Number(screenUI.balance.text) - Number(screenUI.tbalance.text), 2)).toFixed(2);
