package gameCommon.lib 
{
	...
	public class SoundPlayer 
	{
		...
		public static var _introMuted:Boolean=false;
		...
		//private static var _currenSoundName:String="";
		private static var _muteVar:Boolean = false;
		...
		static public function backgroundSound(evt:Event):void
		{
		...
		static public function playSound(soundName:String,numLoops:int = 0,fetchbyurl:Boolean = true):void
		{
			...
			else
			{
				...
				if (_soundMuted)
					...
					//if (_currenSoundName != "")
					// stopSound(_currenSoundName);
			     
				  // _currenSoundName = soundName;
				 if (!_muteVar || soundName=="buttonClick")
				 {
		...
		static public function getSoundByUrl(name:String):Sound{
	     	  ...
	     	  var url:String = _initUrl + _nameClassMap[name].url as String;
			//  trace(url);
