package gameCommon.lib 
{
	...
	//	private var emoticons:Array = [":$", ":9", ":b", ":s",":p",":)",":("];
		...
		public function ChatRenderer(chatField:TextField,inpField:TextField,csf:Function = null,emoRadius:Number = 18) 
		{
	        ...
	        _chatField.mouseWheelEnabled = false;
		...
		{			
			...
			{
				var str:String = _inputField.text;
				if (str.length > 0 && _chatSubmitFunc != null)
				{
					_chatSubmitFunc(str);
		...
		public function parseForEmoticons():void
		{
			...
			//str = normalText;
			//trace (normalText, "normal text is ");
			//trace(str,"html text is +++");
		...
		//		{
			...
			//	}
				...
				emo.x = charBounds.x  * 98 / 100 - 3;
				emo.y = charBounds.y + 2* int (charBounds.y/300) - int (charBounds.y/1200);
				addChild(emo);
				//str += " " + emo.y;
